...
# Method to draw a single card and remove it from the deck
def draw_card(cards)
...
# Method to calculate a hand's value, including allowance for aces high or low
...
#Method to display the total for the player
...
# Establish initial hands and display their values
  player_hand = draw_first_hand(player_hand, cards)
  display_hand(player_hand)
  comp_hand = draw_first_hand(comp_hand, cards)
  player_sum = value_of_hand(player_hand)
  comp_sum = value_of_hand(comp_hand)
  display_total(name, player_sum)

# Player has choice to (h)it or (s)tay
  choice = "h"
  until choice == "s" || player_sum > 21
...
# Conditions to end the game: player busts, player wins, tie, or dealer wins
  if player_sum > 21
